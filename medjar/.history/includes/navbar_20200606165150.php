<link rel = "icon" type = "image/png" href = "images/logo.png">

</head>
    <body>
        <!-- nav-->
        <nav class="navbar navbar-expand-lg navbar-light sticky-top" id="navibar">
            <a class="navbar-brand" href="index.php"><img src="images/logonapi.png" height="35" width="70" class="d-inline-block"></a>
            <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNavAltMarkup" aria-controls="navbarNavAltMarkup" aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse justify-content-end" id="navbarNavAltMarkup">
                <div class="navbar-nav">
                    <a class="nav-item nav-link active" href="jaroslawiec.php">Jarosławiec</a>
                    <a class="nav-item nav-link active" href="noclegi.php">Noclegi</a>
                    <a class="nav-item nav-link active" href="cennik.php">Cennik</a>
                    <a class="nav-item nav-link active" href="galeria.php">Galeria</a>
                    <a class="nav-item nav-link active" href="#sec4">Kontakt</a>
                </div>
            </div>
        </nav>
<!-- end nav-->

<!--Section1-->
<section id="sec1" style="min-height: 60vh">
    <div id="carouselGallery" class="carousel slide" data-ride="carousel" style="min-height: 60vh">
            <ol class="carousel-indicators">
                <li data-target="#carouselGallery" data-slide-to="0" class="active"></li>
                <li data-target="#carouselGallery" data-slide-to="1"></li>
                <li data-target="#carouselGallery" data-slide-to="2"></li>
            </ol>
        <div class="carousel-inner" style="min-height: 60vh">
            <div class="carousel-item active" style="min-height: 60vh">
                <img src="images/slide111.jpeg" class="d-block w-100" alt="...">
                <div class="carousel-caption d-none d-md-block">
                    <h2>NOCLEGI JAROSŁAWIEC</h2>
                </div>
            </div>
            <div class="carousel-item" style="min-height: 60vh">
                <img src="images/slide111.jpeg" class="d-block w-100" alt="...">
                <div class="carousel-caption d-none d-md-block">
                    <h2>NOCLEGI JAROSŁAWIEC</h2>
                </div>
            </div>
            <div class="carousel-item" style="min-height: 60vh">
                <img src="images/slide111.jpeg" class="d-block w-100" alt="..." >
                <div class="carousel-caption d-none d-md-block">
                    <h2>NOCLEGI JAROSŁAWIEC</h2>
                </div>
            </div>
        </div>
        <a class="carousel-control-prev" href="#carouselGallery" role="button" data-slide="prev">
            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
            <span class="sr-only">Previous</span>
        </a>
        <a class="carousel-control-next" href="#carouselGallery" role="button" data-slide="next">
            <span class="carousel-control-next-icon" aria-hidden="true"></span>
            <span class="sr-only">Next</span>
        </a>
    </div>
</section>
        <!-- end sec1 -->